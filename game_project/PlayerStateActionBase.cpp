...
PlayerStateActionBase::PlayerStateActionBase(int& modelHandle,int& bottomHandle,
    ...
    nowAnimState({ NULL }),
    oldAnimState({ NULL }),
    nowAnimState_bottom({NULL}),
    oldAnimState_bottom({NULL})
{
    ...
    AnimState& oldAnimState,AnimState& nowAnimState)
{
    ...
    if (oldAnimState.AttachIndex != -1)
    {
        MV1DetachAnim(modelHandle, oldAnimState.AttachIndex);
